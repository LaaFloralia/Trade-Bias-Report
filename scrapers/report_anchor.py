...
#
# 2026-08-12 の XAUUSD 特化再設計以降の扱い:
#   - 構造/流動性マップ/Premium-Discount/未充填FVG は AI 内部利用専用
#     (執行プランの価格帯根拠・スコア #3 構造整合・前回照合)。
#     レポート本文にレベル一覧として再掲しない (プロンプト側の出力ルールで禁止)。
#   - ボラ/ATR百分位・COT百分位 は旧 /xau-tf 単体レポート廃止に伴い
#     Daily/Weekly が本文で引用する正規ソースになった (エンジンにしかない計算値)。
XAU_TF_SECTIONS = [
    (["マルチタイムフレーム構造"], "構造"),
    (["流動性マップ"], "流動性マップ"),
