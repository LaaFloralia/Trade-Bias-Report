...
| レポート品質の 3 修正（2026-08-14、社長レビュー起点） | ①**スコア表記の機械強制**: `scrapers/score_consistency.py` が内訳表を再集計してセクション0 を訂正する（LLM が「N と読み替えること」注記で代替するのを許さない。1 ページ目だけを見る運用で誤った信頼度に基づく執行を防ぐ）。訂正が走った事実は run ログと Telegram 通知に出す ②**前回照合の時刻フィルタ**: `report_anchor` が前回レポートのヘッダから発行時刻を抽出し、スイープ各件に `[発行後 = 照合可]` / `[発行前 = 照合対象外]` を付与。発行前の値動きを前回予測の的中根拠にすることを禁止（Bias-Review-Log の verdict が系統的に甘くなるのを防ぐ）③**プール距離を近端〜遠端に**: 帯中心の単一値では現値を跨ぐ帯（例 BSL 4,353.29–4,407.63 に対し現値 4,354.66）が「+0.73%」と表示され、帯のどこを指すか読めなかった。`現値を内包` の明示と合わせて修正 |
| IMF 中銀累計の検算可能性（2026-08-14） | `cumulative_3m_t` は確定月 3 ヶ月の合計だが内訳表示は速報月を含む直近 3 ヶ月のため、累計対象月が内訳に現れないことがあった（+136.4t に対し表示は M06+M05 = +77.5t）。`cumulative_periods` を追加し、対象月レンジと「内訳に出ていない確定月」を明示 |
