...
## 全文詳細レイヤー（2026-08-18 追加）

初版は 1 ページ目だけが設計され、後半が素の markdown 変換（見出しと段落の羅列）のままだった。
**前半だけ設計された文書は、設計されていないのと同じ**（読者は結局後半で迷子になる）。
そこで詳細側にも同じ密度の構造を与えた。すべて「同じテキストを要素で包む」変換に限定している。

| 変換 | 目的 |
|---|---|
| 節の索引（冒頭） | 10〜16 ページ文書での現在地把握 |
| キッカー付き節見出し（`セクション3` / `ポジショニング` の 2 段） | 節の境界を明示。見出し語を捨てずに小見出しへ回す |
| `**ラベル:** 本文` → ラベル付きボックス（タグ列 38mm 固定） | 段落の壁からラベルを引き剥がし、本文の開始位置を揃える |
| 段落全体が太字 → 強調文ボックス | 結論・訂正など文書内の判断を目立たせる |
| テーブルセルの判定語 → バッジ（20 字以下のトークンのみ） | Bullish / 当たり / 無相関化 等を一目で拾う |
| スコア列（ヘッダに「点」）→ 符号付きチップ | ダッシュボードと同じ視覚言語を詳細側にも適用 |
| 相関係数セル → 数値の下にミニバー | −1〜+1 のどこにいるかを数値と併記 |
| 数値セル → 右揃え + tabular-nums | 桁の比較を可能にする |
| 列数 7 以上の表 → フォント一段小 / 見出しは nowrap | 「チェ / ック」のような列見出しの分断を防ぐ |
| リスト前の空行補完（`_normalize_md`） | `**ラベル:**` 直後の箇条書きがハイフンのまま本文に露出する不具合の修正 |

**情報保存の契約（最重要）**: 上記の変換で本文が 1 文字も失われないことを
`tests/test_human_report.py::test_detail_preserves_every_source_line` が機械検証する。
MD の全行から構文記号（`#`/`*`/`|`/コロン等の区切り）を落とした文字列が、
レンダリング結果のテキストに含まれることを全行について確認する。
方向記号（▲▼◆）は本文と混ざらないよう `<span class="mark">` に分離してあり、
検証時に除外される。**この契約があるので、見た目の改善が情報欠落に化けない。**
