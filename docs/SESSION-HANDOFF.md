...
- Trial 7 (23:50): GVZ from TradingView adopted (22.44, 9/25; FRED 23.59 kept alongside). Parent review found an empty FedWatch table when all probabilities are missing (runtime `numeric_validation.facts_markdown` now prints 取得不可 instead; patch `20260927-fedwatch-empty-table.patch`) and missing provenance in the price figure (`97c9b97`: subtitle is the price's observation time; caption and links name MyFXBook and its unconfirmed observation time).
- Trial 8 (9/27 00:07, Sunday): Investing week tabs did not switch, so only the day view ("No Events Scheduled") was read and the calendar figure was missing → `ac79f62` reloads once when both weeks yield nothing and records missing tabs. A live re-scrape returned 14 events.
- Trial 9 (9/27 00:26): `succeeded_local` / `parent_passed`, no issues. Jev news triage 31 → 9 (2.4 s), summary audit 1 request 279 ms $0.00004 (12/12 exact_match/supports), GVZ from TradingView, calendar 14 events on the first attempt.
- Unrelated uncommitted change in the working tree: `config.yaml` `output.gdrive_pdf_dir` (another session) makes `tests/test_config_ssot.py::test_output_setting_accessor_safe_on_missing_keys` fail; not touched.
