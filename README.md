...
### 8-3. 出力後の品質ガード（2026-08-14 追加）

生成された MD をそのまま保存せず、機械検証を 1 段挟む。

| ガード | 実装 | 目的 |
|---|---|---|
| 統一スコアの整合 | `scrapers/score_consistency.py` | セクション0 の `信頼度 / スコア` を 8 項目内訳表の再集計値に一致させる。LLM が「セクション0 は N と読み替えること」という注記で代替した場合も、行そのものを訂正して注記を落とす。訂正時は run ログ（`score_check`）と Telegram 通知に出す。再集計値が Low（プラン非提示）に落ちる場合だけは本文構成まで変わるため自動訂正せず `needs_regeneration` として警告する |

前回照合の妥当性は入力側で担保する。`report_anchor` が前回レポートのヘッダ（`生成: MM/DD HH:MM JST`）から
発行時刻を抽出し、`### リテール分析` のスイープ各件に `[発行後 = 照合可]` / `[発行前 = 照合対象外]` を付ける。
前回予測より前に起きた値動きを的中根拠にすると Bias-Review-Log の verdict が系統的に甘くなるため、
プロンプト側でも発行前イベントの使用を禁止している。
